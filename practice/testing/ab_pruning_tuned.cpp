...
public:
    ...
    int getSearchDepth() {
        if (turn_count <= 5) return 5;
        if (turn_count <= 10) return 6;
        if (turn_count <= 15) return 6;
        if (turn_count <= 20) return 7;
        return 8;
    }
    
    int getMaxCandidates(int depth) {
        // Consider top 15 candidates at all levels
        return 15;
    }
    ...
    pair<int, vector<int>> simulate(int alpha, int beta, bool is_max_player, int depth, bool opponent_passed = false) {
        ...
        // Use ordered moves for better alpha-beta pruning
        vector<Move> ordered_moves = generateOrderedMoves(is_max_player);
        
        // Prune low-quality moves: only consider top candidates
        int max_candidates = min((int)ordered_moves.size(), getMaxCandidates(depth));
        
        for (int i = 0; i < max_candidates; i++) {
            const Move& move = ordered_moves[i];
