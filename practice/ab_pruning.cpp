...
const int BASE_SEARCH_DEPTH = 4;
const int DEPTH_INCREASE_STEP = 10;
const int DEPTH_INCREAMENT = 1;
...
public:
    ...
    int getSearchDepth() {
        return BASE_SEARCH_DEPTH + (turn_count / DEPTH_INCREASE_STEP) * DEPTH_INCREAMENT;
    }
    ...
    pair<int, vector<int>> simulate(int alpha, int beta, bool is_max_player, int depth, bool opponent_passed = false) {
        ...
        if (opponent_passed && ((is_max_player && current_board_value > 0) || (!is_max_player && current_board_value < 0))) {
            return {current_board_value, MOVE_PASS};
        }

        // If opponent passed and they're losing, pass to end the game
        if (opponent_passed && ((is_max_player && current_board_value >= 0) || (!is_max_player && current_board_value <= 0))) {
            return {current_board_value, MOVE_PASS};
        }
        ...
        if ((is_max_player && current_board_value > 0) || (!is_max_player && current_board_value < 0)) {
            is_terminal = false;
            auto rv = simulate(alpha, beta, !is_max_player, depth + 1, true);
    ...
    vector<int> calculateMove(int _myTime, int _oppTime) {
        ...
        if (opponent_last_passed) {
            ...
            if (current_score >= 0) {  // We're winning or tied
                return MOVE_PASS;
            }
        }
